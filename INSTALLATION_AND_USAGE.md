...
**Expected output:**
```
Signal Sentinel Scanner v3.0.1
```
...
| Tag | Description |
|-----|-------------|
| `3.0.1` | Specific version (recommended for CI/CD) |
...
```bash
docker run --rm ghcr.io/signalcoding/signal-sentinel-scanner:3.0.1 --version
```
...
| Image | `ghcr.io/signalcoding/signal-sentinel-scanner:3.0.1` |
...
docker run --rm \
  ...
  ghcr.io/signalcoding/signal-sentinel-scanner:3.0.1 --discover --skills
```
...
docker run --rm \
  ...
  ghcr.io/signalcoding/signal-sentinel-scanner:3.0.1 --config /config/mcp.json
```
...
**Docker:**
```bash
docker run --rm ghcr.io/signalcoding/signal-sentinel-scanner:3.0.1 \
...
docker run --rm \
  -v "$HOME/.cursor:/home/sentinel/.cursor:ro" \
  -v "$(pwd):/output" \
  ghcr.io/signalcoding/signal-sentinel-scanner:3.0.1 \
...
*Document generated for Signal Sentinel Scanner v3.0.1*
